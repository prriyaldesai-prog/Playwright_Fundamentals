...
```
Error: locator.check: Error: strict mode violation: getByText('Male', { exact: true }) resolved to 2 elements:
    1) <label class="control-chip">…</label> aka getByLabel('Gender').getByText('Male', { exact: true })
    2) <em>Male</em> aka getByLabel('Practice 10').getByText('Male', { exact: true })
...
> 42 |             await page.getByText(data.Gender, { exact: true }).check();
  ...
  57 | 
  58 | 
  59 |             await page.getByRole("radiogroup", { name: data.Profession }).check();
