...
- Name: Projects\Project_11_DataDrivenTesting\Task_ProfileFill_DataDrivenTesting1_pagefill.spec.ts >> DDT >> DDL profile register fill - VihanaDesai
...
Call log:
  - waiting for getByRole('radiogroup', { name: 'Manual Tester' }).first()
...
> 22 |             await page.screenshot({ path: `out/fail-${testInfo.title}.png`, fullPage: true });
  ...
  42 |             await page.getByText(data.Gender).first().check();
