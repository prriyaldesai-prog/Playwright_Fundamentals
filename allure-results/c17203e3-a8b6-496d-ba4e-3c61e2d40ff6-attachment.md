...
```
Error: locator.check: Error: strict mode violation: getByText('Male') resolved to 6 elements:
    1) <label class="control-chip">…</label> aka getByLabel('Gender').getByText('Male', { exact: true })
    2) <label class="control-chip">…</label> aka getByText('Female', { exact: true })
    3) <span class="tta-mark-chip is-test">…</span> aka getByText('data-testid=gender-male /')
    4) <em>Male</em> aka getByLabel('Practice 10').getByText('Male', { exact: true })
    5) <code>getByRole('radio', { name: 'Male' }).check()</code> aka getByText('getByRole(\'radio\', { name: \'Male\' }).check()', { exact: true })
    6) <span class="token-string">'Male'</span> aka getByText('\'Male\'', { exact: true })
...
> 42 |             await page.getByText(data.Gender).check();
     |                                               ^ Error: locator.check: Error: strict mode violation: getByText('Male') resolved to 6 elements:
